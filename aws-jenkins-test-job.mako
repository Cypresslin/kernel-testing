...
if 'instance_type' in data:
    deploy  = '$KT/cl create %s $SUT %s "%s" --type %s' % (data['cloud'], data['series_name'], data['region'], data['instance_type'])
else:
...
${data['description']}
    ...
    <properties>
        <hudson.plugins.throttleconcurrents.ThrottleJobProperty>
            <categories>
                <string>${data['instance_type']}</string>
                <string>${data['instance_type'].split('.')[0]}</string>
            </categories>
